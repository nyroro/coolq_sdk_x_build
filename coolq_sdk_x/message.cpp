...
namespace cqplus::message {
        ...
        for (auto it = msg_str.cbegin(); it != end; ++it) {
            ...
            params:
                if (curr == ']') {
                    ...
                    boost::split(params, (const string&)params_s.str(), boost::is_any_of(","));
    ...
    int64_t Message::send(const Target &target) const {
        ...
        CQPLUS_API_PARAMS["msg"] = (const std::string&)(*this);
        if (target.group_id.has_value()) {
            CQPLUS_API_PARAMS["group_id"] = target.group_id.value();
            CQPLUS_API_INVOKE("send_group_msg");
        } else if (target.discuss_id.has_value()) {
            CQPLUS_API_PARAMS["discuss_id"] = target.discuss_id.value();
            CQPLUS_API_INVOKE("send_discuss_msg");
        } else if (target.user_id.has_value()) {
            CQPLUS_API_PARAMS["user_id"] = target.user_id.value();
            CQPLUS_API_INVOKE("send_private_msg");
        }
        return CQPLUS_API_RET.asInt64();
    }
