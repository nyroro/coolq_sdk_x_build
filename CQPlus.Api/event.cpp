...
namespace cqplus::event {
    std::function<Json::Value(const Json::String&, const Json::Value&)> on_api_invoke;
}

__CQPLUS_EVENT(const char*, api_invoke, 8)
(const char* method, const char* params) {
    ...
    ret = cqplus::utils::call_if_valid(cqplus::event::on_api_invoke, _method, _params);
...
__CQPLUS_EVENT(void, api_free, 4)
(void* p) {
    if (p)
    {
        free(p);
